...
public static class Settings
{
  ...
  public static bool PreyIsNtr { get; set; }
  public static int ProgressCount { get; set; }
  public static int NtrProgressCount { get; set; }
  ...
  public static ConfigFile LoadConfiguration()
  {
    ...
    PreyIsNtr = configFile.Bind("Ability", "PreyIsNtr", false, "See Prey as Mischief instead of Seduce.\nMischief has special text, short progress count and doesn't cancel when being attacked etc."
      +"\n将 捕食 视作 恶作剧\n恶作剧具有独特文本，更短的回合数，并且不会在被攻击时取消，等等").Value;
    ProgressCount = configFile.Bind("Ability", "ProgressCount", 25, "Sex progress(round) count. A smaller number indicates a faster action.\n超人行动回合计数，数字越小意味着行为越快").Value;
    NtrProgressCount = configFile.Bind("Ability", "NtrProgressCount", 10, "Mischief progress(round) count. A smaller number indicates a faster action.\n恶作剧行动回合计数，数字越小意味着行为越快").Value;
